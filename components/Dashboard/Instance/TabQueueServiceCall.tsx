...
const TabQueueServiceCall = ({ id }: { id: number }) => {
  ...
  const [searchInputService, setSearchInputService] = useState("");
  ...
  const fetchAudio = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/panggilantrian/get/${slug}`,
        {
          method: "POST",
          headers: {
  ...
  const replayAudio = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/panggilantrian/re-call/${idQueue}`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${Cookies.get("token")}`,
  ...
  return (
    <>
      <div className="mb-5">
        <Select value={slug} onValueChange={(e) => setSlug(e)}>
          ...
          <SelectContent className="pt-10">
            <div className="px-2 fixed border-b w-full top-0 flex items-center justify-between z-10">
              ...
              <Input
                ...
                onChange={(e) => setSearchInputService(e.target.value)}
              />
            </div>
            <SelectGroup>
              <SelectLabel>Layanan</SelectLabel>
              {result2?.map((item: any) => (
...
export default TabQueueServiceCall;
